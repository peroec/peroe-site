...
import type { Route } from "./+types/forum.auth.register";

export function meta() {
  return [{ title: "注册 | 论坛 | 二叉树树" }];
}
...
export default function Register(_props: Route.ComponentProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.token) throw new Error(data.message || "注册失败");
  ...
  return (
    <main className="mx-auto flex w-full max-w-sm flex-col px-4 py-20">
      <h1 className="mb-8 text-center text-2xl font-bold text-white">注册</h1>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          <label className="mb-1.5 block text-sm text-muted">密码（至少 6 位）</label>
          <input
            type="password"
            required
            minLength={6}
      ...
      <p className="mt-6 text-center text-sm text-muted-2">
        已有账号？
        <Link
          to={`/forum/auth/login?redirect=${encodeURIComponent(redirect)}`}
